...
fct_expt_dir=~/linkguardian/switch_impl/expt_scripts/fct_expt_rdma_fast
...
ssh caelus-ae "sudo ip netns exec lg $fct_expt_dir/bin/rdma_simple -s 10.2.2.2" > /dev/null & # server
...
cecho "YELLOW" "Starting RDMA client on lumos..."
    # rdma write client 
    sudo ip netns exec lg $mydir/bin/rdma_simple -c 10.2.2.2 -m ${flow_size} -l ~/traces/fct_expt_rdma/$expt_name.log -n $num_flow_trials
# done

# Step 3: Stop the RDMA server on caelus and client on lumos
echo ""
cecho "YELLOW" "Stopping RDMA server on caelus..."
ssh caelus-ae "sudo killall -2 rdma_simple"
