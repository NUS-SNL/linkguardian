...
/* Timestamp */
struct timespec ts_start, ts_finish, ts_start_meta_xchange;
...
static int client_prepare_connection(struct sockaddr_in *s_addr)
{
     ...
     * to a local device. */
    ret = rdma_resolve_addr(cm_client_id, NULL, (struct sockaddr *)s_addr, 100);
    if (ret)
    {
...
static int client_xchange_metadata_with_server()
{
    ...
    /* Now we post it */
    // getchar();
    clock_gettime(CLOCK_REALTIME, &ts_start_meta_xchange);
 ...
 * 2) RDMA read from remote bufer -> dst (NOT USING CURRENTLY)
 */
static int client_remote_memory_ops(FILE *logf)
{
    struct ibv_wc wc;
    int ret = -1;
    ...
    debug("client_remote_memory_ops\n");
...
{
    printf("Usage:\n");
    printf("rdma_client: [-a <server_addr>] [-p <server_port>] [-s string (e.g., 'test') or -f filename in /src (relative dir to current))] [-l filename to record timestamp (relative dir to current, e.g., /log.txt] [-n <number to run> - optional, default 1]\n");
...
int main(int argc, char **argv)
{
    ...
    /* buffers are NULL */
    src = dst = NULL;
    /* to prevent string+file duplicated input */
    int dupinput = 0;
    int num_to_run = 1;
    /* Parse Command Line Arguments */
    printf("---Start parsing...\n");
    while ((option = getopt(argc, argv, "f:s:a:p:l:n:")) != -1)
    {
        switch (option)
        {
        case 's':
        ...
        case 'f':
            ...
            {
                debug("Current working dir: %s, with len %u\n",
                      cwd, /* current directory */
                      (unsigned int)strlen(cwd) /* len */);
            }
            else
            {
            ...
            /* read file */
            char *filepath = concat(cwd, optarg);
            FILE *fptr = fopen(filepath, "r");
            if (fptr == NULL)
            {
                rdma_error("Failed to open data file %s \n", filepath);
                return 1;
            }

            char txtdata[MAX_DATA_LEN];
            unsigned int txtdatalen = 0;
            debug("Input file data:\n");
            while ((txtdata[txtdatalen] = fgetc(fptr)) != EOF && txtdatalen < MAX_DATA_LEN)
            {
                // debug("%c", data[txtdatalen]);
                txtdatalen = txtdatalen + 1;
            }
            debug("\n\n");
            printf("WRITE file size: %u \n", txtdatalen);

            /* allocate src and dst */
            src = calloc(txtdatalen, 1);
            if (!src)
            {
                rdma_error("Failed to allocate memory : -ENOMEM\n");
                return -ENOMEM;
            }
            /* Copy the passes arguments to src */
            strncpy(src, txtdata, txtdatalen);
            dst = calloc(txtdatalen, 1);
            if (!dst)
            {
                rdma_error("Failed to allocate destination memory, -ENOMEM\n");
                free(src);
        ...
        case 'p':
            ...
            break;
        case 'l':
            /* logname to save file */
            if (getcwd(cwd, sizeof(cwd)) != NULL)
            {
                debug("Current logging dir: %s, with len %u\n",
                      cwd, /* current directory */
                      (unsigned int)strlen(cwd) /* len */);
            }
            else
        ...
        case 'n':
            ...
            num_to_run = strtoul(optarg, NULL, 0);
            break;
        default:
            usage();
            break;
    ...
    if (dupinput != 1)
    {
        /* either put both -s and -f options, or no input streams */
        if (src == NULL)
            printf("Please provide a <string -s > or <filename -f > to copy \n");
        else if (dupinput == 2)
            printf("Please provide only one of <string -s > or <filename -f > \n");
        usage();
    }
