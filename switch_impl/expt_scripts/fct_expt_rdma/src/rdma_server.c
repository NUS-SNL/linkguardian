...
int main(int argc, char **argv) {
    printf("*** Start server running (infinite loop) ***\n\n");

    int ret, option;
    struct sockaddr_in server_sockaddr_fixed;
    bzero(&server_sockaddr_fixed, sizeof server_sockaddr_fixed);
    server_sockaddr_fixed.sin_family = AF_INET;                /* standard IP NET address */
    server_sockaddr_fixed.sin_addr.s_addr = htonl(INADDR_ANY); /* passed address */
    /* Parse Command Line Arguments, not the most reliable code */
    while ((option = getopt(argc, argv, "a:p:")) != -1) {
        switch (option) {
            case 'a':
                /* Remember, this will overwrite the port info */
                ret = get_addr(optarg, (struct sockaddr *)&server_sockaddr_fixed);
                if (ret) {
            ...
            case 'p':
                ...
                server_sockaddr_fixed.sin_port = htons(strtol(optarg, NULL, 0));
                break;
            default:
                usage();
                break;
    ...
    while (1) {
        printf("Server run %d\n", nLoop++);
        if (!server_sockaddr_fixed.sin_port) {
            /* If still zero, that mean no port info provided */
            server_sockaddr_fixed.sin_port = htons(DEFAULT_RDMA_PORT); /* use default port */
        }
